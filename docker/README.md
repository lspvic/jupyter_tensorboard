![docker pulls](https://img.shields.io/docker/pulls/lspvic/tensorboard-notebook.svg) ![docker stars](https://img.shields.io/docker/stars/lspvic/tensorboard-notebook.svg) [![](https://images.microbadger.com/badges/image/lspvic/tensorboard-notebook.svg)](https://microbadger.com/images/lspvic/tensorboard-notebook "lspvic/tensorboard-notebook image metadata")
...
```
docker run -d -p 8888:8888 lspvic/tensorboard-notebook start-notebook.sh --NotebookApp.password='sha1:74ba40f8a388:c913541b7ee99d15d5ed31d4226bf7838f83a50e'
```

For example, to set the base URL of the notebook server, run the following:

```
docker run -d -p 8888:8888 lspvic/tensorboard-notebook start-notebook.sh --NotebookApp.base_url=/some/path
```

For example, to disable all authentication mechanisms (not a recommended practice):

```
docker run -d -p 8888:8888 lspvic/tensorboard-notebook start-notebook.sh --NotebookApp.token=''
```
...
docker run -d -p 8888:8888 \
    ...
    lspvic/tensorboard-notebook start-notebook.sh \
...
```
docker run -it --rm lspvic/tensorboard-notebook start.sh ipython
```

Or, to run JupyterLab instead of the classic notebook, run the following:

```
docker run -it --rm -p 8888:8888 lspvic/tensorboard-notebook start.sh jupyter lab
```
